...
class LaneDetectorCV:
    def __init__(self):
        ...
        # Obstacle detection
        ###############################################################################
        self.is_front_obs = False
        self.is_side_obs = False
        self.is_in_avoidence = 0
        self.turning_time = 0
        rospy.Subscriber("/perception/obstacle_if_side", Int32MultiArray, self.obs_detect_callback)
    ...
    def obs_detect_callback(self, msg):
        self.is_front_obs = bool(msg.data[0])
        self.is_side_obs = bool(msg.data[1])
    ...
    def generate_waypoints(self, lane_mask):
        ...
        """
        if self.is_front_obs and self.is_in_avoidence==0:
            self.is_in_avoidence=1
            # 設定曲線參數
            while self.is_front_obs:
                if self.turn_dir == 'left':
                    curve = self.avoidance_curve_left
                    curve_1=curve[:int(len(curve)/2)]  # extract the first half
                ...
                if self.avoidance_step < len(curve_1):
                    x, y = curve_1[self.avoidance_step]
                    ...
                    for i in range(self.avoidance_step, len(curve_1)):
                        pt = PoseStamped()
                        pt.header.frame_id = "map"
                        pt.pose.position.x, pt.pose.position.y = curve_1[i]
                    ...
                    return path, [], []
                elif self.avoidance_step >= len(curve_1):
                    x, y = curve_1[-1]
                    ...
                    self.turning_time = self.avoidance_step
                    print("new virtual waypoints & endgoal has been published: ", self.avoidance_step)
                    print(x, y)
                    print(len(path.poses))
                    path, [], []
                
        elif self.is_in_avoidence == 1:  # drive in the second lane
            pass_obs = False
            self.avoidance_step = 0
            self.last_avoidance_time = rospy.Time.now()
            ...
            if not pass_obs and self.is_side_obs:
                pass_obs=True
            if (not self.is_side_obs) and pass_obs:
                self.is_in_avoidence =2
            return path, left_boundary, right_boundary
                
        elif self.is_in_avoidence == 2:
            self.avoidance_step = 0
            while self.avoidance_step <= self.turning_time:
                if self.turn_dir == 'left':
                    curve = self.avoidance_curve_left
                    curve_1=curve[int(len(curve)/2):]  # extract the first half
    ...
    def generate_curve(self, direction='left', num_points=30):
        ...
        if self.camera_matrix:
            ...
            cx = 1225 / 2
        x_curve = cx + sign * (1.0 * self.scale * np.sin(np.linspace(0, 4 * np.pi, num_points)))

...
class LaneNetDetector:
    ...
    def __init__(self, path_to_weights='../../../weights/yolopv2.pt'):
        ...
        self.estimated_lane_width_pixels = 700  # Approximate lane width in image pixels # 500 and 750
        self.prev_endgoal = None  # Previous waypoints for temporal consistency
    ...
    def img_callback(self, img):
        ...
        try:
            ...
            # white mask
            # lower_white_hsv = np.array([0, 0, 200])        # H: arbitrary, S: super low, V: high
            # upper_white_hsv = np.array([180, 40, 255])

            # yellow mask
            # lower_yellow_hsv = np.array([15, 80, 80])      # H = 15 is between orange and yellow
            # upper_yellow_hsv = np.array([35, 255, 255])    # H =35 is between green and yellow 
            ...
            if len(self.frame_buffer) >= self.buffer_size:
                ...
                for i, contrasted_img in enumerate(original_images):
                    ...
                    annotated_img = self.detect_lanes(seg[i], ll[i], contrasted_img, binary)
    ...
    def detect_lanes(self, seg, ll, img, img_gray):
        ...
        t = ll.max().item() * 0.3  # applying 40% of the maximun value as the threshold
        # ll_seg_mask = lane_line_mask(ll, threshold=t)
        ll_seg_mask = lane_line_mask(ll, threshold=0.15)

        # Transform the image into birdeye's view
        ll_seg_mask = self.region_of_interest(ll_seg_mask, mode='bird')
        img_gray = self.region_of_interest(img_gray, mode='bird')
        ...
        if len(waypoints.poses) < 25 and self.is_obs == False:
            waypoints, left_boundary, right_boundary = self.generate_waypoints(img_gray)

        # Draw waypoints on image for visualization
        img_with_waypoints = self.draw_waypoints(self.region_of_interest(img, mode='bird').copy(), waypoints, left_boundary, right_boundary)
...
if __name__ == "__main__":
    try:
        ...
        print("[INFO] Lane detector initialized. Now using YOLO+CV.")

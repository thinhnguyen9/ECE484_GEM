...
class LaneNetDetector:
    ...
    def __init__(self, path_to_weights='../../../weights/yolopv2.pt'):
        ...
        self.using_one_lane = 'right' # left, or right, or undesignated
        self.estimated_lane_width_pixels = 600  # Approximate lane width in image pixels
    ...
    def img_callback(self, img):
        ...
        try:
            ...
            img_no_white = cv2.bitwise_and(img, img, mask=white_mask)
            img_gray = cv2.cvtColor(img_no_white, cv2.COLOR_BGR2GRAY)
            
            # Apply contrast enhancement using thresholding
            threshold = 180
            mask = img_gray >= threshold
            dimmed_gray = (img_gray * 0.1).astype(np.uint8)  # Reduce brightness of non-lane areas
    ...
    def generate_waypoints(self, lane_mask):
        ...
        left_boundary, right_boundary = [], []  # Store left boundary points
        offset_pixels = 600  # Estimated distance from left boundary to lane center
        ...
        for lb, rb in zip(left_boundary, right_boundary):
            if self.using_one_lane == 'left':
                if lb:
                    x_center = int(lb[0] + offset_pixels // 2)
                    y_center = lb[1]
                else:
                    continue
            elif self.using_one_lane == 'right':
                if rb:
                    x_center = int(rb[0] - offset_pixels // 2)
                    y_center = rb[1]
                else:
                    continue
            elif self.using_one_lane == 'undesignated':
                if lb and rb:
                    x_center = int((lb[0] + rb[0]) / 2.0)
...
if __name__ == "__main__":
    try:
        ...
        print("[INFO] Lane detector initialized. Now using starter code.")
